...
class SCMessageVC: MessagesViewController {
    ...
    let titleLabel = SCDefaultLbl(x: 5,y: 0 ,width: 100, height: 25, fontSize: 16, fontWeight: .medium, textAlignment: .left)
    let subTitleLabel = SCDefaultLbl(x: 5,y: 22 ,width: 100, height: 24, fontSize: 16, fontWeight: .medium, textAlignment: .left)
    ...
    init(chatId: String, recipientId: String, recipientName: String, chatAvatar: String) {
    ...
    private func creatTypingObserver(){
        FTypingListener.shared.creatTypingIndicator(chatRoomId: chatId) { isTyping in
    ...
    func send(text: String? , photo: UIImage? , video: Video? , location: String? , audio: String? , audioDuration: Float = 0.0 ){
        ...
        Outgoing.sendMessage(chatId: chatId, text: text, photo: photo, audio: audio, video: video , location: location, memberIds: [User.currentId , recipientId])
    ...
    private func markMessageAsRead(_ localMessage: LocalMessage){
        ...
        if localMessage.senderId != User.currentId {
            ...
            FMessageListener.shared.updateMessageStatues(localMessage, userId: recipientId)
        }
    }
    ...
    //MARK: - listen for read Statues
    
    private func listenForReadStatuesUpdated(){
        
        FMessageListener.shared.listenForReadStatues(User.currentId, collectionId: chatId) { updatedMessage in
            
            self.updateReadStatues(updatedMessage)
        }
    }
    ...
    //MARK: - update Read Statues
    
    private func updateReadStatues(_ updatesLocalMessage: LocalMessage){
        
        for index in 0 ..< mkMessages.count {
            
            let tempMessage = mkMessages[index]
            
            if updatesLocalMessage.id == tempMessage.messageId {
                
                mkMessages[index].status = updatesLocalMessage.status
                mkMessages[index].readDate = updatesLocalMessage.readDate
                
                RealmManager.shared.save(updatesLocalMessage)
...
extension SCMessageVC: GalleryControllerDelegate{
